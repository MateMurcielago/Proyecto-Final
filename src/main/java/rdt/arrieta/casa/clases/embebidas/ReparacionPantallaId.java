package rdt.arrieta.casa.clases.embebidas;

import jakarta.persistence.*;
import java.io.Serializable;
import java.util.Objects;

@Embeddable
public class ReparacionPantallaId implements Serializable {
    @Column(name = "id_reparacion")
    private Long id_reparacion;

    @Column(name = "id_codigo_pantalla")
    private Long id_codigo_pantalla;

    public ReparacionPantallaId() {}

    public ReparacionPantallaId(Long id_reparacion, Long id_codigo_pantalla) {
        this.id_reparacion = id_reparacion;
        this.id_codigo_pantalla = id_codigo_pantalla;
    }

    public Long getId_reparacion() {
        return id_reparacion;
    }

    public Long getId_codigo_pantalla() {
        return id_codigo_pantalla;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if(!(o instanceof ReparacionPantallaId)) return false;
        ReparacionPantallaId that = (ReparacionPantallaId) o;
        if(!Objects.equals(id_reparacion, that.id_reparacion)) return false;
        return Objects.equals(id_codigo_pantalla, that.id_codigo_pantalla);
    }

    @Override
    public int hashCode() {
        int result = id_reparacion != null ? id_reparacion.hashCode() : 0;
        result = 31 * result + (id_codigo_pantalla != null ? id_codigo_pantalla.hashCode() : 0);
        return result;
    }
}
